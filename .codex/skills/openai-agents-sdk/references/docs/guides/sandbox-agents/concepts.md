...
- [Agents as tools](/openai-agents-js/guides/tools#4-agents-as-tools): expose multiple sandbox agents as tools, usually by passing a sandbox run config on each `asTool(...)` call so each tool gets its own sandbox boundary.
